 ...
 */
public abstract class DepthFirst extends Search
{
    private static Result depthFirst(Node node, Node goal)
    {
        Node nextChild;
        int i;
        Vector<Node> route;
            ...
            if (node.getChildren() != null)
            {
                        ...
                        &&(!found))
                {
                    ...
                    if(!memory.contains(nextChild.getState()))
                        // recursive call
                        result = depthFirst(nextChild, goal);
                    //end if
                    //end else
                    i = i + 1;
                }//end while
